...
//        };
        ...
        phones.add(new Phone("삼성1", 20000));
        phones.add(new Phone("삼성2", 20000));
        phones.add(new Phone("삼성3", 20000));

        try(ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("phone.txt"))){
//            oos.writeInt(phones.length);
//            for( int i = 0 ; i < phones.length; i++ ){
//                oos.writeObject(phones);
//            }
            for(int i = 0;i<phones.size();i++){
                oos.writeObject(phones.get(i));
            }
...
//                phones[i] = (Phone) oos.readObject();
//            }

//            System.out.println("파일 읽기 완료! 저장된 객체들:");
//            for (Phone phone : phones) {
//                System.out.println(phone);
//            }
            int i = 0;
            while(true){
                phonelist.add((Phone)oos.readObject());
